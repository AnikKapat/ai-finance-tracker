...
export const processRecurringTransaction = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    await step.run("process-transaction", async () => {
      ...
      if (!transaction) {
        console.log("Recurring transaction not found");
        return;
      }
      ...
      if (!transaction.recurringInterval || !transaction.nextRecurringDate) {
        console.error(
          "Recurring transaction is missing scheduling information:",
          transaction.id,
        );
        return;
      }
      ...
      await db.$transaction(async (tx) => {
        ...
        // Claim this occurrence first.
        // If another worker already processed it, nothing happens.
...
export const generateMonthlyReports = inngest.createFunction(
  ...
  async ({ step }) => {
    console.log("========== MONTHLY REPORT STARTED ==========");
